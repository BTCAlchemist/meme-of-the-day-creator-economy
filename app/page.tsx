import { getMemeOfDay, MOCK_MEMES, getTopCreators } from "@/lib/data";
...
export default function HomePage() {
  const memeOfDay = getMemeOfDay();
  const recentMemes = MOCK_MEMES.filter((m) => !m.isMemeOfDay).slice(0, 4);
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8 space-y-12">
      ...
      {/* Recent + Top creators side by side */}
      ...
      <section className="bg-gradient-to-r from-bags/10 to-accent/10 border border-bags/20 rounded-2xl p-8 text-center">
        <div className="flex items-center justify-center gap-2 mb-3">
          ...
          <h2 className="text-xl font-black text-white">
            Creator Economy via Bags
          </h2>
        </div>
        <p className="text-gray-400 max-w-md mx-auto mb-5">
          Every creator on MemeDay has their own token on Bags. Invest early,
          earn with them as they grow. No VC, no middlemen — just memes and
          markets.
        </p>
